...
\gls{TILDA} provides longitudinal data across five waves for community dwelling adults in Ireland. This thesis uses data from five core \gls{TILDA} waves (Waves 1--5). For contextual comparison and benchmarking against international cohorts (see chapter~\ref{ch:chapter1}), an additional external resource, the harmonised \gls{TILDA} dataset, was also inspected and prepared. The harmonised dataset, which has been pre aligned to international standards such as the \gls{G2G}, contains a subset of variables mapped to a common cross study format and thus differs from the raw \gls{TILDA} files. Although the harmonised dataset had already been aligned to a common international standard and was simpler to process, it was still subjected to the thesis's variable filtering and numerical standardisation steps to ensure compatibility with the five wave cleaned datasets prior to modelling.
...
\begin{itemize}
    ...
    \item \textbf{Harmonised:} 2016 (release)
\end{itemize}
...
Before processing, raw \gls{TILDA} datasets were manually inspected, and clear noninformative variables (e.g., administrative identifiers, constant columns, high missingness) were removed. Table~\ref{tab:raw_dataset_shape} shows the raw dimensions of each dataset.
...
Manual filtering used a rule of thumb prior to automated cleaning:
\begin{itemize}
